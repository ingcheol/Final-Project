...
---
## 페이지 번역
글로벌 사용자를 위해 Spring AI를 활용한 실시간 번역 파이프라인을 구축 버튼 하나로 모든 UI 텍스트가 즉시 현지화되어 제공
...
## 자가진단
사용자가 사진분석(선택) 및 본인의 증상을 입력(텍스트, 음성)하면 용자의 증상 키워드를 추출해 **벡터 DB(Vector DB)**에 미리 인덱싱된 전문 의학 가이드라인을 검색 자가 문진을 시작 AI가 RAG 기술을 통해 종합진단을 실시하여 진료과를 추천
